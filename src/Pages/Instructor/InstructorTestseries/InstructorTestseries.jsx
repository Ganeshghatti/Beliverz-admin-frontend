...
import "./InstructorTestseries.scss";
...
export default function InstructorTestseries() {
      ...
      try {
        if (instructor.isInstructor) {
          ...
          const response = await axios.get(
            `${SERVER_URL}/instructor/get-accessible-testseries`,
            {
              headers: {
  ...
  return (
    <div className="InstructorTestseries flex">
      <Panel tab="Testseries" />
                ...
                >
                  ...
                  <CardContent className="flex flex-col gap-2 items-center">
                    <p className="font-bold text-2xl md:text-xl text-center">
                      You don't have access to any testseries
                    </p>
                    <p className="font-medium text-lg md:text-base text-center">
                      Please request admin to grant access to testseries
                    </p>
                  </CardContent>
                </Card>
              )}
              {alltestseries.map((item, index) => (
                <Link to={`/instructor/testseries/${item.testseriesId}`}>
                  <div
                    className="cursor-pointer relative instructor-testseries-card gap-1 rounded-xl flex flex-col items-center"
